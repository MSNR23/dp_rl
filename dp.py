...
# 重心までの長さ（m_hand + 投擲物）
lg1 = l1 / 2
lg2 = (m_forearm * (l_forearm / 2) + (m_hand + 7.0) * (l_forearm + l_hand / 2)) / (m_forearm + (m_hand + 7.0))
...
# CSVファイルの保存先ディレクトリ
save_dir1 = r'max_R_bow'
save_dir2 = r'try_bow'
save_dir3 = r'maxQ_bow'
save_dir4 = r'finQ_bow'
...
def update_world(q1, q2, q1_dot, q2_dot, tau, action):
    ...
    if action == 0:
        ...
        tau = np.array([[0.0], [40.0]])
    elif action == 3:
        tau = np.array([[0.0], [-40.0]])
    elif action == 4:
        tau = np.array([[60.0], [40.0]])
    elif action == 5:
        tau = np.array([[-60.0], [-40.0]])
    elif action == 6:
        tau = np.array([[60.0], [-40.0]])
    ...
    if q2 <= 0:
        tau[1, 0] += 60.0  # 0度のとき、正の方向に5N
    elif q2 >= np.radians(150):
        tau[1, 0] += -60.0  # 145度のとき、負の方向に5N
...
def compute_reward(q1, q2, q1_dot, q2_dot, cumulative_energy):
    ...
    reward -= 0.1 * cumulative_energy  # 累積消費エネルギーによるペナルティ
